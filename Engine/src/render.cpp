...
void drawTriangle(Vec2 v0, Vec2 v1, Vec2 v2, Color color, float angle) {
    ...
    if(angle == 0.0f) {
        ...
        float width = std::max({v0.x, v1.x, v2.x}) - std::min({v0.x, v1.x, v2.x}) + 1;
        float height = std::max({v0.y, v1.y, v2.y}) - std::min({v0.y, v1.y, v2.y}) + 1;
...
// Draw Text
void drawText(float x, float y, const char *msg, Uint8 r, Uint8 g, Uint8 b, Uint8 a) {
    stringRGBA (renderer, x, y, msg, r, g, b, a);
}


void drawText(Vec2 pos, const char *text, Color color, Font font, float ptSize, float angle) {
    Texture texture = getTextTexture(renderer, text, font, ptSize, {color.r, color.g, color.b, color.a});

    SDL_FRect dst = {pos.x, pos.y, texture.src.width, texture.src.height};

    if(angle == 0.0f) {
        SDL_RenderTexture(renderer, texture.texture, NULL, &dst);
    } else {
        SDL_FPoint centre = {texture.texture->w/2.0f, texture.texture->h/2.0f};

        // Render texture (rotated)
        SDL_RenderTextureRotated(renderer, texture.texture, NULL, &dst, angle, &centre, SDL_FLIP_NONE);
    }
}

void drawText(Vec2 pos, const char *text, Uint8 r, Uint8 g, Uint8 b, Uint8 a, Font font, float ptSize, float angle) {
    drawText(pos, text, {r, g, b, a}, font, ptSize, angle);
}

void drawText(float x, float y, const char *text, Color color, Font font, float ptSize, float angle) {
    drawText({x, y}, text, color, font, ptSize, angle);
}

void drawText(float x, float y, const char *text, Uint8 r, Uint8 g, Uint8 b, Uint8 a, Font font, float ptSize, float angle) {
    drawText({x, y}, text, {r, g, b, a}, font, ptSize, angle);
}
